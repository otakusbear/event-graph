...
<a-button @click="toggleCausalRelations">显示/隐藏因果关系</a-button></div>
    ...
    <div ref="network2" id="network2"></div>
    <div class="tlcontainer" style="position:absolute;display: none;top: 40%;left: 10%;width:80%;height:50%;overflow-x: auto;overflow-y: hidden;scroll-behavior: smooth;">
  ...
  <p class="name">{{ event.name }}</p>
    ...
    <div id="timecontainer" style="z-index:2;display: block;" >
...
// },
  async mounted() {
    ...
    // 初始化图形
    const container = this.$refs.network2;
    const data = {
      nodes: this.nodes,
      edges: this.edges,
    };
...
// });
  this.loadData();
    ...
    handleSelectChange() {
      this.selectedEventType=this.$store.state.title;
  ...
  while (true) {
    ...
    let match = buffer.match(/\{[\s\S]*"event":[\s\S]*?}/);
