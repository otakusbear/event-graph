<template>
  <div>
    ...
    <h2>选择事件主题:</h2>
    <select v-model="selectedEventType" @change="getEventData">
      <option v-for="eventType in eventTypes" :key="eventType" :value="eventType">{{ eventType }}</option>
    </select>
    <br>
    <button @click="showSubevents">显示子事件</button>
    <button @click="hideSubevents">隐藏子事件</button>
    <button @click="toggleTimeRelations">显示/隐藏时序关系</button>
<button @click="toggleCausalRelations">显示/隐藏因果关系</button>
...
export default {
  ...
  async mounted() {
    // 获取事件类型列表
    const response = await axios.get('/api/old-types');
  ...
  methods: {
    async getEventData() {
  // 获取事件数据
  const response = await axios.get('/api/old-data', {
