...
for (let i = 0; i < triples.value.length; i++) {
    ...
    if (subject !== 'NON') {
        ...
        if (!subjectNode) {
            ...
            subjectNode = { id: nodeId++, label: '  ', color: { background: "pink" }, font: { size: 100 } };
            nodesArray.push(subjectNode);
            nodeMap.set(subject, subjectNode);
            // Create a new text node for the subject
        const subjectTextNode = { physics:false, id: nodeId++, label: subject, shape:'text', x:subjectNode.x, y:subjectNode.y, font:{size :35} };
        textNodesArray.push(subjectTextNode);
        }
    }
    ...
    if (!verbNode) {
        ...
        verbNode = { id: nodeId++, label: '  ', color: { background: "lightblue" }, font: { size: 150 } };
        nodesArray.push(verbNode);
        nodeMap.set(verb, verbNode);

        // Create a new text node for the verb
        const verbTextNode = { physics: false, id: nodeId++, label: verb, shape: 'text', x: verbNode.x, y: verbNode.y, font: { size: 40 } };
    ...
    if (object !== 'NON') {
        ...
        if (!objectNode) {
            ...
            objectNode = { id: nodeId++, label: '  ', color: { background: "pink" }, font: { size: 100 } };
            nodesArray.push(objectNode);
            nodeMap.set(object, objectNode);
            // Create a new text node for the object
        const objectTextNode = { physics:false,id :nodeId++,label :object ,shape:'text',x :objectNode.x,y :objectNode.y,font:{size :35}};
        textNodesArray.push(objectTextNode);
        }
    }

    // Create edges between the subject, verb, and object nodes
    if (subject !== 'NON') {
        edgesArray.push({ from: subjectNode.id, to: verbNode.id, color: { color: '#b8b0b0' }, arrows: { to: false }, shadow: false });
    }
    if (object !== 'NON') {
        edgesArray.push({ from: verbNode.id, to: objectNode.id, color: { color: '#b8b0b0' }, arrows: { to: false }, shadow: false });
    }
  ...
  data() {
    return {
      ...
      textNodesArray: this.textNodesArray,
      //   关系线数组
  ...
  methods: {
    updateTextNodesPosition() {
    // Get the positions of all nodes in the network
    const positions = this.network.getPositions();

    // Update the position of each text node
    for (const textNode of this.textNodesArray) {
    console.log(this.nodeMap);
      const verbNode = this.nodeMap.get(textNode.label);
      const position = positions[verbNode.id];
      textNode.x = position.x;
      textNode.y = position.y + 5;
    }

    // Update the data set
    this.nodes.update(this.textNodesArray);
  },

    init() {
      let _this = this;
      //1.创建一个nodes数组
      console.log(_this.nodesArray);
      console.log(_this.textNodesArray);
      _this.nodes = new Vis.DataSet(_this.nodesArray, _this.textNodesArray);
      //2.创建一个edges数组
  ...
  _this.updateTextNodesPosition();
    _this.network.on('dragEnd', () => {
    // 调用updateTextNodesPosition函数
    _this.updateTextNodesPosition();
});
    ...
    resetAllNodes() {
      ...
      _this.nodes.add(_this.textNodesArray);
